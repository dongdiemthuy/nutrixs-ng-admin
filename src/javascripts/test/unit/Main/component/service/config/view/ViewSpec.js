...
define(function (require) {
    ...
    describe("Service: View config", function () {
        ...
        it('should returns the identifier.', function () {
            var view = new View();
            view
                .addField(new Field('postid').identifier(true))
                .addField(new Field('name').identifier(false));

            expect(view.identifier().name()).toEqual('postid');
        });
        ...
        it('should map some raw entities', function () {
            ...
            var entries = view.mapEntries([
                { postid: 1, title: 'Hello', published: true},
                { postid: 2, title: 'World', published: false},
                { postid: 3, title: 'How to use ng-admin', published: false}
            ]);
        ...
        it('should map some one entity when the identifier in not in the view', function () {
            ...
            entity
                .identifier(new Field('postid'));

            var entry = view.mapEntry({ postid: 1, title: 'Hello', published: true});
